# Lista de figuras

Esta lista consolida las figuras actualmente seleccionadas e integradas en la base documental de la tesis. La numeración sigue el orden ya fijado en los borradores de `EDA` y `Arquitectura e implementación del sistema`.

## Lista consolidada

1. `Figura 1. Ventas promedio, promociones, transacciones y petróleo.`
2. `Figura 2. Distribución de ventas en festividades nacionales.`
3. `Figura 3. Distribución de ventas en festividades regionales.`
4. `Figura 4. Distribución de ventas en festividades locales.`
5. `Figura 5. Patrones de ventas por variables de calendario.`
6. `Figura 6. Arquitectura vigente del sistema de predicción e integración con Onyx.`

## Trazabilidad

| Figura | Archivo | Sección/capítulo |
|---|---|---|
| `Figura 1` | [eda_01_sales_covariates.png](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/figures/eda/eda_01_sales_covariates.png) | [eda.md](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/eda.md) |
| `Figura 2` | [eda_02_national_holidays.png](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/figures/eda/eda_02_national_holidays.png) | [eda.md](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/eda.md) |
| `Figura 3` | [eda_03_regional_holidays.png](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/figures/eda/eda_03_regional_holidays.png) | [eda.md](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/eda.md) |
| `Figura 4` | [eda_04_local_holidays.png](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/figures/eda/eda_04_local_holidays.png) | [eda.md](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/eda.md) |
| `Figura 5` | [eda_05_calendar_patterns.png](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/figures/eda/eda_05_calendar_patterns.png) | [eda.md](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/eda.md) |
| `Figura 6` | [architecture_01_current_system.png](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/figures/architecture/architecture_01_current_system.png) | [arquitectura_implementacion.md](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/arquitectura_implementacion.md) |

## Nota editorial

- Esta lista es suficiente para una versión actual del manuscrito con base en los capítulos ya documentados.
- La numeración vigente quedó consolidada en [MANUSCRIPT_NUMBERING.md](/mnt/c/Users/juana/PycharmProjects/tesis-umpe-supermarket-stock-prediction/docs/MANUSCRIPT_NUMBERING.md).
- Si más adelante se incorporan figuras de resultados o reproducibilidad, habrá que actualizar primero la numeración maestra antes de modificar referencias cruzadas.
