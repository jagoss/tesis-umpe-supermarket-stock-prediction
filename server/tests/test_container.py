...
class TestSelectModel:
    def test_dummy_backend(self) -> None:
        settings = Settings(
            model_backend="dummy", model_path="", default_prediction_value=5.0,
            cors_origins=["*"], log_format="text", log_level="INFO",
            api_key="", rate_limit="60/minute", max_horizon_days=365,
        )
    ...
    def test_unknown_backend_raises(self) -> None:
        settings = Settings(
            model_backend="unknown", model_path="", default_prediction_value=0.0,
            cors_origins=["*"], log_format="text", log_level="INFO",
            api_key="", rate_limit="60/minute", max_horizon_days=365,
        )
